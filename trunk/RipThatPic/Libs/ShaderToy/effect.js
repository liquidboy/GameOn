...
function EffectPass(gl, precission, supportDerivatives, callback, obj, forceMuted, forcePaused, quadVBO, outputGainNode, id) {
    ...
    //this.FinalColorTexture = CreateTextureRGB8LinearClamp( gl, xres, yres ); //iqiq
    //this.FinalFBO = createFBO( gl, this.FinalColorTexture ); //iqiq
...
EffectPass.prototype.NewShader_Image = function (gl, shaderCode) {
    ...
    if (n1 > 0 || n2 > 0 || n3 > 0) {
        ...
        var res = CreateShader(gl, vsSource, this.mHeader + shaderCode + this.mImagePassFooterVR, false);
        if (res.mSuccess == false)
            return res.mInfo;
...
EffectPass.prototype.NewTexture = function (wa, gl, slot, url) {
    ...
    if (url == null) {
        ...
        if (typeof navigator.getUserMedia === "undefined") {
            alert("Shadertoy: Webcam not implement in this browser");
            texture.mForceMuted = true;
        }

        if (texture.mForceMuted) {
            texture.globject = gl.createTexture();
            createGLTextureLinear(gl, null, texture.globject);
            texture.loaded = true;
        }
        else {
        ...
        if (wa == null || typeof navigator.getUserMedia === "undefined") {
            if (!this.mForceMuted) alert("Shadertoy: Web Audio not implement in this browser");
            texture.mForceMuted = true;
        }
        ...
        if (texture.mForceMuted) {
            ...
            navigator.getUserMedia({ "audio": true },
                                    function (stream) {
                                        ...
                                        texture.mFreqData = new Uint8Array(texture.mAnalyser.frequencyBinCount);
                                        texture.mWaveData = new Uint8Array(texture.mAnalyser.frequencyBinCount);
                                        texture.loaded = true;
                                    },
                                    function (error) {
