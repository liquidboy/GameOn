    ...
    (function (Directives) {
        ...
        var ShaderToyDirective = (function () {
            ...
            ShaderToyDirective.prototype.dataLoadShader = function (jsnShader) {
                ...
                //    if (this.sc.res.mPublished == 0) {
                //        sp.selectedIndex = 3;
                //    }
                //    else if (this.sc.res.mPublished == 1) {
                //        sp.selectedIndex = 1;
                //    }
                //    else if (this.sc.res.mPublished == 2) {
                //        sp.selectedIndex = 2;
                //    }
                //    else if (this.sc.res.mPublished == 3) {
                //        sp.selectedIndex = 0;
                //    }
                //}
                //updateLikes();
            ...
            ShaderToyDirective.prototype.loadShader = function (gShaderID) {
                ...
                //}
                try {
                    var res = '[{ "ver": "0.1", "info": { "id": "4t23RR", "date": "1426631078", "viewed": 1101, "name": "Toon Cloud", "username": "AntoineC", "description": "Simple 2D layered animation. Better in full screen (could become a screen saver?) ", "likes": 43, "published": 1, "flags": 0, "tags": ["2d", "minimalist"], "hasliked": 0 }, "renderpass": [{ "inputs": [], "outputs": [], "code": "\/\/ ----------------------------------------------------------------------------------------\n\/\/\t\"Toon Cloud\" by Antoine Clappier - March 2015\n\/\/\n\/\/\tLicensed under:\n\/\/  A Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License.\n\/\/\thttp:\/\/creativecommons.org\/licenses\/by-nc-sa\/4.0\/\n\/\/ ----------------------------------------------------------------------------------------\n\n#define TAU 6.28318530718\n\n\nconst vec3 BackColor\t= vec3(0.0, 0.4, 0.58);\nconst vec3 CloudColor\t= vec3(0.18,0.70,0.87);\n\n\nfloat Func(float pX)\n{\n\treturn 0.6*(0.5*sin(0.1*pX) + 0.5*sin(0.553*pX) + 0.7*sin(1.2*pX));\n}\n\n\nfloat FuncR(float pX)\n{\n\treturn 0.5 + 0.25*(1.0 + sin(mod(40.0*pX, TAU)));\n}\n\n\nfloat Layer(vec2 pQ, float pT)\n{\n\tvec2 Qt = 3.5*pQ;\n\tpT *= 0.5;\n\tQt.x += pT;\n\n\tfloat Xi = floor(Qt.x);\n\tfloat Xf = Qt.x - Xi -0.5;\n\n\tvec2 C;\n\tfloat Yi;\n\tfloat D = 1.0 - step(Qt.y,  Func(Qt.x));\n\n\t\/\/ Disk:\n\tYi = Func(Xi + 0.5);\n\tC = vec2(Xf, Qt.y - Yi ); \n\tD =  min(D, length(C) - FuncR(Xi+ pT\/80.0));\n\n\t\/\/ Previous disk:\n\tYi = Func(Xi+1.0 + 0.5);\n\tC = vec2(Xf-1.0, Qt.y - Yi ); \n\tD =  min(D, length(C) - FuncR(Xi+1.0+ pT\/80.0));\n\n\t\/\/ Next Disk:\n\tYi = Func(Xi-1.0 + 0.5);\n\tC = vec2(Xf+1.0, Qt.y - Yi ); \n\tD =  min(D, length(C) - FuncR(Xi-1.0+ pT\/80.0));\n\n\treturn min(1.0, D);\n}\n\n\n\nvoid mainImage( out vec4 fragColor, in vec2 fragCoord )\n{\n\t\/\/ Setup:\n\tvec2 UV = 2.0*(fragCoord.xy - iResolution.xy\/2.0) \/ min(iResolution.x, iResolution.y);\t\n\t\n\t\/\/ Render:\n\tvec3 Color= BackColor;\n\n\tfor(float J=0.0; J<=1.0; J+=0.2)\n\t{\n\t\t\/\/ Cloud Layer: \n\t\tfloat Lt =  iGlobalTime*(0.5  + 2.0*J)*(1.0 + 0.1*sin(226.0*J)) + 17.0*J;\n\t\tvec2 Lp = vec2(0.0, 0.3+1.5*( J - 0.5));\n\t\tfloat L = Layer(UV + Lp, Lt);\n\n\t\t\/\/ Blur and color:\n\t\tfloat Blur = 4.0*(0.5*abs(2.0 - 5.0*J))\/(11.0 - 5.0*J);\n\n\t\tfloat V = mix( 0.0, 1.0, 1.0 - smoothstep( 0.0, 0.01 +0.2*Blur, L ) );\n\t\tvec3 Lc=  mix( CloudColor, vec3(1.0), J);\n\n\t\tColor =mix(Color, Lc,  V);\n\t}\n\n\tfragColor = vec4(Color, 1.0);\n}\n\n\n", "name": "", "description": "", "type": "image" }] }]';
                    var code = '\/\/ ----------------------------------------------------------------------------------------\n\/\/\t\"Toon Cloud\" by Antoine Clappier - March 2015\n\/\/\n\/\/\tLicensed under:\n\/\/  A Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License.\n\/\/\thttp:\/\/creativecommons.org\/licenses\/by-nc-sa\/4.0\/\n\/\/ ----------------------------------------------------------------------------------------\n\n#define TAU 6.28318530718\n\n\nconst vec3 BackColor\t= vec3(0.0, 0.4, 0.58);\nconst vec3 CloudColor\t= vec3(0.18,0.70,0.87);\n\n\nfloat Func(float pX)\n{\n\treturn 0.6*(0.5*sin(0.1*pX) + 0.5*sin(0.553*pX) + 0.7*sin(1.2*pX));\n}\n\n\nfloat FuncR(float pX)\n{\n\treturn 0.5 + 0.25*(1.0 + sin(mod(40.0*pX, TAU)));\n}\n\n\nfloat Layer(vec2 pQ, float pT)\n{\n\tvec2 Qt = 3.5*pQ;\n\tpT *= 0.5;\n\tQt.x += pT;\n\n\tfloat Xi = floor(Qt.x);\n\tfloat Xf = Qt.x - Xi -0.5;\n\n\tvec2 C;\n\tfloat Yi;\n\tfloat D = 1.0 - step(Qt.y,  Func(Qt.x));\n\n\t\/\/ Disk:\n\tYi = Func(Xi + 0.5);\n\tC = vec2(Xf, Qt.y - Yi ); \n\tD =  min(D, length(C) - FuncR(Xi+ pT\/80.0));\n\n\t\/\/ Previous disk:\n\tYi = Func(Xi+1.0 + 0.5);\n\tC = vec2(Xf-1.0, Qt.y - Yi ); \n\tD =  min(D, length(C) - FuncR(Xi+1.0+ pT\/80.0));\n\n\t\/\/ Next Disk:\n\tYi = Func(Xi-1.0 + 0.5);\n\tC = vec2(Xf+1.0, Qt.y - Yi ); \n\tD =  min(D, length(C) - FuncR(Xi-1.0+ pT\/80.0));\n\n\treturn min(1.0, D);\n}\n\n\n\nvoid mainImage( out vec4 fragColor, in vec2 fragCoord )\n{\n\t\/\/ Setup:\n\tvec2 UV = 2.0*(fragCoord.xy - iResolution.xy\/2.0) \/ min(iResolution.x, iResolution.y);\t\n\t\n\t\/\/ Render:\n\tvec3 Color= BackColor;\n\n\tfor(float J=0.0; J<=1.0; J+=0.2)\n\t{\n\t\t\/\/ Cloud Layer: \n\t\tfloat Lt =  iGlobalTime*(0.5  + 2.0*J)*(1.0 + 0.1*sin(226.0*J)) + 17.0*J;\n\t\tvec2 Lp = vec2(0.0, 0.3+1.5*( J - 0.5));\n\t\tfloat L = Layer(UV + Lp, Lt);\n\n\t\t\/\/ Blur and color:\n\t\tfloat Blur = 4.0*(0.5*abs(2.0 - 5.0*J))\/(11.0 - 5.0*J);\n\n\t\tfloat V = mix( 0.0, 1.0, 1.0 - smoothstep( 0.0, 0.01 +0.2*Blur, L ) );\n\t\tvec3 Lc=  mix( CloudColor, vec3(1.0), J);\n\n\t\tColor =mix(Color, Lc,  V);\n\t}\n\n\tfragColor = vec4(Color, 1.0);\n}\n\n\n';
                    var code2 = code.replace('\/', '/');
                    var res2 = '[{ "ver": "0.1", "info": { "id": "4t23RR", "date": "1426631078", "viewed": 1101, "name": "Toon Cloud", "username": "AntoineC", "description": "Simple 2D layered animation. Better in full screen (could become a screen saver?) ", "likes": 43, "published": 1, "flags": 0, "tags": ["2d", "minimalist"], "hasliked": 0 }, "renderpass": [{ "inputs": [], "outputs": [], "code": "", "name": "", "description": "", "type": "image" }] }]';
                    var jsnShader = JSON.parse(res2);
                    jsnShader[0].renderpass[0].code = code2;
        ...
        var ShaderToy = (function () {
            function ShaderToy(playerElement, editorElement, passElement) {
                ...
                this.newScriptJSON = function (jsn) {
                    try {
                ...
                this.refreshTexturThumbail = function (myself, slot, img, forceFrame, gui, guiID, time, passID) {
                    ...
                    if (img == null) {
                        ...
                        if (guiID == 2) {
                            ...
                            var pb = document.getElementById("myPauseButton" + slot);
        ...
        var Effect = (function () {
            function Effect(vr, ac, gl, xres, yres, callback, obj, forceMuted, forcePaused) {
                this.NewTexture = function (passid, slot, url) {
        ...
        var EffectPass = (function () {
            function EffectPass(gl, precission, supportDerivatives, callback, obj, forceMuted, forcePaused, quadVBO, outputGainNode, id) {
                this.NewShader = function (gl, shaderCode) {
                    if (gl == null)
                        return "No GL";
                    var res = null;
                    if (this.mType == "sound")
                ...
                this.Create_Image = function (wa, gl) {
                    this.MakeHeader();
                ...
                this.Destroy_Image = function (wa, gl) {
                };
                this.Create_Sound = function (wa, gl) {
                    this.MakeHeader();
                ...
                this.Destroy_Sound = function (wa, gl) {
                ...
                this.MakeHeader = function (precission, supportDerivatives) {
                    if (this.mType == "image")
                ...
                if (url == null) {
                    ...
                    texture.image.onload = function () {
                        ...
                        if (url.mSrc == "/presets/tex14.png")
                            this.createGLTextureNearest(gl, texture.image, texture.globject);
                        else if (url.mSrc == "/presets/tex15.png")
                            this.createGLTextureNearestRepeat(gl, texture.image, texture.globject);
                        else
                            this.createGLTexture(gl, texture.image, format, texture.globject);
                        texture.loaded = true;
                        if (me.mTextureCallbackFun != null)
